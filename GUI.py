...
class Interface(QMainWindow):
    ...
    def create_interface(self, language='es'):
        ...
        if not self.portugues_action:
            ...
            self.language_group.addAction(self.portugues_action)
        ...
        chain_label = QLabel(_("Velocidad"))
        ...
        verify_button = QPushButton(_("Verificar"))
        verify_button.clicked.connect(self.process())
    ...
    def verify_word(self, word):
        status=self.automata.get_initial_status()
        self.update_nodes(status)
        for symbol in word:
            if (status, symbol) in self.automata.get_transitions():
                self.update_edges(status, self.automata.get_transitions()[(status, symbol)])
                status = self.automata.get_transitions()[(status, symbol)]
                self.update_nodes(status)
            else:
                return False
        return status in self.automata.get_final_status()
    
    def process(self):
        if self.verify_word(self.string_linee.text()):
            self.process_speech(self.traduction("La palabra es aceptada"))
            QMessageBox.information(self, self.traduction("Resultado"), self.traduction("La palabra es aceptada"))
        else:
            self.process_speech(self.traduction("La palabra no es aceptada"))
